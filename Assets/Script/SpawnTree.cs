...
public class SpawnTree : MonoBehaviour
{
    ...
    public float distance = 10f;                    //Distance used for the Z so it's always placed in front
    ...
    public void Update()
    {
        ...
        if (Input.GetMouseButtonDown(0) && select)
        {
            ...
            select = false;                                         //Toggle button is instantly turned off.
    ...
    {
        if (select) toggle.GetComponent<Image>().color = Color.red; //Tree place mode
        else toggle.GetComponent<Image>().color = Color.white;      //Normal Button coloe
    }
}
